...
lstm step1 MSE:15.69
![](image/lstm_epoch10_step5.png)
...
- 小波结果来编码股票嵌入向量（Embedding），可以从不同的时间和频率域提供股票相关性的解释。意味着我们可以使用小波分析这一数学工具来处理和分析股票数据，将股票和其相关特征（如价格变动、交易量等）转换为能够表示其内在特性和关系（时域+频域关系）的向量形式。
