...
旋转矩阵量化的核心思想：将权重矩阵与旋转矩阵相乘，以减少异常值并提高可量化性。

量化方法总结：
- 按通道量化：主要用于模型权重和 KV 缓存的 Key 张量，解决通道间分布差异问题
- 按 Token 量化：主要用于 KV 缓存的 Value 张量和输入激活值，适应动态序列特性
- KV 缓存混合应用（KIVI）：Key 按通道量化，Value 按 Token 量化，两者协同优化显存与精度
