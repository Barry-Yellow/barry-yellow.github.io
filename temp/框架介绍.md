...
2. [整体架构](#二整体架构)
3. [核心模块详解](#三核心模块详解)
   - [Agent 模块](#31-agent-模块)
   - [LLM 模块](#32-llm-模块)
   - [Knowledge 知识模块](#33-knowledge-知识模块)
   - [Memory 记忆模块](#34-memory-记忆模块)
   - [Context 上下文模块](#35-context-上下文模块)
   - [Workflow 工作流模块](#36-workflow-工作流模块)
   - [Prompt 提示词模块](#37-prompt-提示词模块)
   - [Tool 工具模块](#38-tool-工具模块)
   - [Agent Serve 服务模块](#39-agent-serve-服务模块)
   - [Product 产品层](#310-product-产品层)
...
agentUniverse 是由**蚂蚁集团**开源的多智能体框架，源自其在金融业务中的真实生产实践。蚂蚁集团在处理投资研究、ESG 分析、财报解读、风险评估等复杂金融任务时，发现单一大模型难以满足专业领域的深度要求，因此研发了这套以"多智能体协作"为核心的框架，并将其提炼为通用工具对外开放。

### 1.2 核心定位

agentUniverse 的核心定位是：**帮助开发者构建具备领域专家级别能力的智能体，并通过多智能体协作完成复杂任务。**

它不是一个通用聊天机器人框架，而是专注于：
...
agentUniverse 在专业领域深度上具有明显优势，尤其在金融场景下，其 PEER 协作模式经过学术论文验证，准确率显著优于同类框架。
...
graph TB
    ...
    subgraph 智能体层 ["智能体层 (Agent Layer)"]
        A1[Planning Agent 规划]
        A2[Executing Agent 执行]
        A3[Expressing Agent 表达]
        A4[Reviewing Agent 审查]
        A5[RAG Agent 检索]
        A6[ReAct Agent 推理]
        A7[NL2API Agent]
    end
    ...
    subgraph 基础层 ["基础层 (Base Layer)"]
        B1[配置系统 YAML]
        B2[组件注册中心]
        B3[可观测性 OpenTelemetry]
        B4[数据库 / 向量库]
    end
...
LLM 模块采用**抽象层 + 适配器**设计，上层 Agent 无需关心底层使用哪个模型，只需通过统一接口调用。这使得：
...
当对话历史超出模型上下文窗口时，Memory 模块自动触发压缩：
...
大模型的上下文窗口是有限资源。如何在有限空间内放入最有价值的信息，直接决定输出质量。Context 模块解决的正是这个问题。
...
在多 Agent 系统中，每个 Agent 都有自己的提示词，且提示词质量直接决定输出质量。Prompt 模块解决：
...
多个相关工具可以打包为 **Toolkit**，Agent 可以一次性获得一组能力。例如：
...
规划智能体接收用户的复杂问题，将其分解为若干个可独立执行的子问题。这一步的关键是：
...
若审查不通过，Reviewing Agent 会将具体改进意见反馈给 Planning Agent，触发新一轮迭代。
...
PEER 模式已发表于学术论文《PEER: Expertizing Domain-Specific Tasks with a Multi-Agent Framework and Tuning Methods》（arXiv: 2407.06985）。
...
单一大模型在处理复杂问题时面临"注意力稀释"问题——需要同时关注问题理解、信息检索、逻辑推理、语言表达等多个维度，导致每个维度都不够深入。

PEER 模式通过**专业化分工**解决这个问题：

```
单模型处理复杂问题：
  注意力 = 理解(20%) + 检索(20%) + 推理(20%) + 表达(20%) + 审查(20%)
  每个环节都不够专注 → 质量平庸

PEER 多 Agent 处理：
  Planning Agent：100% 注意力用于任务分解
  Executing Agent：100% 注意力用于信息获取
  Expressing Agent：100% 注意力用于内容综合
  Reviewing Agent：100% 注意力用于质量审查
  每个环节都极度专注 → 质量卓越
```

#### 6.1.2 迭代自我修正机制

Reviewing Agent 的存在使系统具备**自我纠错**能力。这类似于人类专家的工作方式：写完初稿后进行审查，发现问题后修改，直到满意为止。

研究表明，带有自我审查机制的 AI 系统在复杂任务上的准确率比无审查系统高出 15-30%。

#### 6.1.3 RAG 增强的事实准确性

通过将外部知识库与 LLM 结合，RAG 机制显著降低了"幻觉"（hallucination）问题：
...
agentUniverse 不是实验室产物，而是从蚂蚁集团真实金融业务中提炼出来的框架。这意味着：

- 所有设计决策都经过了真实业务场景的检验
- 性能、稳定性、可扩展性都达到了生产级要求
- 边界情况和异常处理都有充分考虑

#### 6.2.2 组件化与可扩展性

框架的每个组件都可以独立替换，这带来了极高的灵活性：
...
所有组件通过 YAML 配置文件定义，业务逻辑与框架代码解耦：
...
agentUniverse 支持将行业专家知识系统化地注入 Agent：

- 通过知识库注入专业文档和规则
- 通过 Few-shot 示例注入专家判断模式
- 通过 Opinion-inject（DOE 模式）直接注入专家观点
- 通过微调数据集（PEER 金融数据集）优化模型行为

#### 6.3.2 可解释性与可审计性

金融场景对 AI 决策的可解释性有严格要求。agentUniverse 通过以下机制提供可解释性：

- 每个 Agent 的输入输出都有完整记录
- 工具调用链路清晰可追溯
- Reviewing Agent 的评分和改进意见透明可见
- OpenTelemetry 全链路追踪
...
### 7.1 为什么可观测性在多 Agent 系统中至关重要

多 Agent 系统的复杂性远超单一应用：一个请求可能触发 4-7 个 Agent 的串行/并行执行，每个 Agent 又可能调用多个工具和 LLM。没有完善的可观测性，问题排查将极为困难。

### 7.2 OpenTelemetry 全链路追踪

agentUniverse 深度集成 OpenTelemetry，对以下层面进行全面埋点：
...
agentUniverse 提供多层次的日志能力：

- **结构化日志**：JSON 格式，便于日志分析平台处理
- **分级日志**：DEBUG/INFO/WARNING/ERROR 分级管理
- **云端日志**：支持阿里云日志服务（SLS）等云端日志平台
- **日志 Sink**：可扩展的日志输出目标，支持自定义
...
- **Web 服务**：Flask 框架提供 REST API
- **微服务**：gRPC 支持高性能服务间通信
- **可视化平台**：difizen 画布平台，支持可视化工作流设计
- **监控平台**：OpenTelemetry 兼容任意 APM 系统
